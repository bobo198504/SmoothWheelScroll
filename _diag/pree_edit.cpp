...
#include "anim_core.h"
// MODEL 3.0 lives here: "take how much, give how much" (see the header). It replaces the motion
// above for the live gesture; anim_core.h is kept only for the settings panel's response curve.
...
// MODEL 3.0's two controls.
//
// FINENESS (ms): how long one message's amount takes to be handed to the action, and therefore how
// finely the motion is sliced -- the parameter is named for what the hand feels, not for the
// mechanism. A long window means each turn is broken into many small pieces (very fine); a short
// one means the amount arrives almost at once (blunt, closer to the stock wheel).
//
// It has a FLOOR of 50 ms. Handing a whole notch over in a single call is what the stock wheel
// does, but doing it through this plugin means the amount is re-encoded and pushed into the action
// table, and doing it with no window at all is the one case that has been suspected of wedging the
// UI thread. 50 ms is short enough to feel immediate while still giving the delivery a defined
// window, and it is the blunt end of the range.
static double g_finenessMs = 50.0;
static const double kFinenessMinMs = 50.0, kFinenessMaxMs = 500.0;
static const double kDefaultFinenessMs = 50.0;

// RESISTANCE: how much is taken away per unit time while anything is still owed, measured as
// "how many deltas are taken per 1 ms". It does not look at how fast the wheel turns and it does
// not look at the total: it simply takes that much, every 1 ms, for as long as there is anything
// left to take.
//
// 1 ms is the SAME unit as the animation clock (`kFastTimerMs`), so "one step" is exactly one tick
// of the model: the eating and the payout are on one shared beat, with no separate time base to
// convert between.
//
// That is the whole rule, and it gives the wanted behaviour by arithmetic: a slow turn feeds the
// total slowly, so resistance takes a large share of it; a fast roll feeds it quickly, so
// resistance cannot keep up and nearly all of it is paid out. Faster turn -> less taken, on every
// device, with no speed curve anywhere.
//
// THE SPEED THRESHOLD, and why the range reaches down to zero:
//
//   The take is a fixed rate, and the wheel feeds at 120 deltas per notch. The two balance at a
//   gap of  0.12 / resistance  seconds per notch, so a turn SLOWER than that is eaten faster than
//   it arrives and dies out, while a faster roll piles up and mostly gets through. Measured from a
//   log: at resistance 1.00 the threshold is 120 ms per notch, and the user's ordinary slow rolling
//   (150..500 ms per notch) sat on the eaten side -- one notch at a time barely moved, and speeding
//   up crossed the line and suddenly went. So the useful settings are BELOW 1, and the control
//   reaches 0:
//
//     0.00  = no resistance at all: the total is only eased out (see Fineness);
//     0.25  = threshold 480 ms per notch, so ordinary slow turns keep most of themselves;
//     1.00  = threshold 120 ms per notch;
//     10.00 = threshold 12 ms per notch: only a fast roll survives.
//
// The step is 0.01, because the fine end is where the hand works and a hundredth is the smallest
// change worth feeling.
static double g_resistance = 1.0;
static const double kResistMinUi = 0.0, kResistMaxUi = 10.0;
static const double kDefaultResist = 1.0;

// One resistance step is "one delta per 1 ms", i.e. 1000 deltas per second. The model takes its
// amount in the caller's unit (7-bit units, 8 deltas each), so that is 1000/8 units per second.
static const double kResistDeltasPerSec = 1000.0;
static const double kResistStepMs = 1.0;
static const double kDeltasPerUnit = 120.0 / 15.0; // 8 deltas per 7-bit unit

// MODEL 3.0's parameters: the easing and the resistance. Both are plain numbers the panel sets;
// the resistance is converted from "deltas per 5 ms" into the model's own unit per second.
static anim3::Params AnimParams3()
{
  anim3::Params P;
  P.durationMs = g_finenessMs;
  P.takePerSec = (g_resistance * kResistDeltasPerSec) / kDeltasPerUnit;
  P.snap = (1.0 / kDeltasPerUnit) * 0.5; // half a delta: below what can ever be handed over
  return P;
}
...
// OUTPUT DENSITY
//
//   kFastTimer : drive the hand-over with a 1 ms multimedia timer instead of the
//                plain thread timer (which Windows clamps to ~15.6 ms).
//
// The clock had to become fine for MODEL 3.0: the easing spreads a message's amount
// over a window as short as 10 ms, and a 15.6 ms clock would hand the whole window
// over in ONE late call -- which is not "spread evenly", it is "delivered late", and
// it would make the shortest easing feel worse than native. 1 ms resolves a 10 ms
// window into ~10 evenly spaced slices. timeSetEvent posts to the plugin's own message
// window (never REAPER's), at most one tick outstanding, so the UI thread cannot back up.
//
// The STEP size depends on the receiver (see DeliveryStep). Fine steps are what
// keep a slow roll smooth; a coarser grid inflates it (the "creeps, then jumps" report).
// ---------------------------------------------------------------------------
static const bool kFastTimer = true;    // MODEL 3.0 needs a clock finer than the 15.6 ms thread timer

// Requested period for the multimedia timer.
static UINT kFastTimerMs = 1;
// Fallback thread-timer period, used only if the multimedia timer is unavailable.
static UINT kAnimTimerMs = 5;

// The live wheel gesture is MODEL 3.0 (anim3_core.h): the amount the wheel reports is handed to
// the action unchanged, and the easing only decides WHEN it arrives (see Kick). anim_core.h's
// physics is no longer used by the gesture; it remains only to draw the settings panel's response
// curve, which describes that older model and will be reworked with the controls.
...
// MODEL 3.0 has no "step delivery" threshold any more: the easing itself decides. At ease 0 the
// amount is handed over in one go (which is exactly the stock wheel), and any ease above 0 is a
// real spread -- so there is no release time at which an animation "would not fit a frame" and
// must be short-circuited. That special case belonged to the old model's release slider.
...
// but the relative form carries a FRACTIONAL part as well, so the eased value
...
// (kDeltasPerUnit -- deltas per 7-bit unit, 8 -- is defined up with the resistance, because the
// model parameters need it and those are set before this point.)

// THE DELIVERY STEP, one per filter (see Delivery). Both choose the step by what the RECEIVER can
// actually act on, and both CARRY the remainder to the next frame rather than round it up, so a
// small amount is never inflated into a jump bigger than the wheel reported.
//
//   kStream    -> the relative form's finest step, 1/256 of a 7-bit unit. The form can carry a
//                 fraction, so this is what an ordinary axis gets: the smoothest the form allows.
//   kStepUnits -> one whole 7-bit unit. A whole-unit receiver DROPS anything smaller (measured,
//                 AGENTS.md 21): fed a finer stream it looks dead until enough piles up to cross a
//                 whole unit, then jumps -- exactly the "does not move, then suddenly moves" that
//                 was reported. So it gets units, and the remainder is carried.
static const double kRelSubPerUnit = 256.0; // fractional sub-steps one 7-bit unit can carry
static const double kStreamStepUnits = 1.0 / kRelSubPerUnit; // 1/256 unit = 1/32 delta
static const double kWholeStepUnits = 1.0;                   // one 7-bit unit

// (DeliveryStep, which picks between them, is defined with the Delivery enum below.)

// The relative form tops out at |integer part| = 63..64, so anything larger is split into calls.
static const int kRelIntMax = 63;
...
// ---------------------------------------------------------------------------
// THE FILTER -- how a receiver takes the travel handed to it.
//
// The model is independent of REAPER: it produces an amount of travel and hands it over. What
// differs between receivers is HOW that travel has to arrive, and that difference is the filter.
// Keeping it in ONE function is the point: the action table, the track panel, the piano keys and
// the arrange scrollbars all resolve it the same way, so adding or re-tuning a receiver means
// touching one rule instead of every call site.
//
// Two filters plus the mode, and the distinction that matters is the STEP the receiver can take:
//
//   kStream    -- an ordinary fine-grained receiver. It takes the relative form's finest step, so
//                 the travel is handed over in 1/256-unit pieces and the motion is as smooth as the
//                 form allows.
//   kStepUnits -- a WHOLE-UNIT receiver. It acts on whole 7-bit units and DROPS anything smaller,
//                 so a finer stream leaves it looking dead -- measured on the main view's vertical
//                 axis (AGENTS.md 21): "feeding it a stream below one unit, it drops it and the
//                 axis looks dead". The travel is therefore handed over in whole units, with the
//                 remainder carried to the next frame (never rounded up).
//   kImmediate -- the receiver takes a whole step at once and a coast has nowhere to land, so a
//                 message's amount goes over immediately. The MIDI editor's vertical ZOOM only: a
//                 fixed pixel scale (measured 2 px per step, each call rounding on its own).
//
// The filter decides HOW the travel is handed over. It never changes HOW MUCH travel there is --
// that is the model's decision, and the delivery's job is to hand it over faithfully.
// ---------------------------------------------------------------------------
enum class Delivery
{
  kStream,    // fine-grained: the relative form's finest step, carried between frames
  kStepUnits, // whole 7-bit units only, carried between frames (a receiver that drops less)
  kImmediate  // the whole amount at once, no spread -- a receiver with nowhere to coast
};

// Which axis the gesture drives, and whether it scrolls or zooms.
enum class Axis
{
  kHorizontal = 0,
  kVertical
...
enum class Kind
{
  ...
  // other sort). Its receiver is unknown, so it keeps the normal stream -- the conservative
  // default, and exactly what the previous per-command rule did for such an action.
  kUnknown
};

// The MIDI editor's section id and the two ids this plugin drives by hand (the piano keys and the
// mixer shortcut use them directly). Named once so the table and the call sites cannot disagree.
static const int kMidiSection = 32060;
static const int kMainSection = 0;
static const int kMidiScrollVert = 40432; // View: Scroll vertically
static const int kMidiZoomVert = 40430;   // View: Zoom vertically
static const int kMainScrollVert = 989;   // View: Scroll vertically
static const int kMainZoomVert = 1000;    // View: Zoom vertically
static const int kMainScrollHorz = 977;   // View: Scroll horizontally reversed (a left pan)
static const int kMainZoomHorz = 990;     // View: Zoom horizontally

// THE FILTER: the one place that decides how a receiver is fed (see Delivery above). It is about
// how the travel is handed over, never about how much there is.
//
// WHOLE UNITS, for every axis of the main view. This was settled by reading a log at zero
// resistance, which is the only clean way to see the delivery on its own:
//
//   horizontal scroll   fed the fine stream (1/256 unit per call)  -> "barely moves"
//                       fed whole units (1.00 per call)            -> smooth
//   horizontal zoom     fed the fine stream                        -> "very slow" (yet 99.6% of
//                       the travel HAD been sent -- the receiver just does not act on steps that
//                       small)
//   vertical scroll     fed whole units                            -> smooth
//
// So the receiver acts on whole 7-bit units and ignores finer ones, on BOTH axes and for both
// scrolling and zooming. The form's finest step (1/256 unit) is below what they act on, however
// much of it is sent. That also supersedes the old "vertical = units, horizontal = fine stream"
// split, which was measured back when a notch moved 1.89 units (an eighth of today's 15).
static Delivery FilterFor(int section, Axis axis, Kind kind)
{
  // The MIDI editor keeps its own pair of rules: its vertical ZOOM takes the whole amount at once
  // (a fixed pixel scale with nowhere to coast), its vertical SCROLL takes whole units (AGENTS.md
  // 17). Its horizontal axes are ordinary fine axes.
  if (section == kMidiSection)
  {
    if (axis == Axis::kHorizontal)
      return Delivery::kStream;
    return (kind == Kind::kZoom) ? Delivery::kImmediate : Delivery::kStepUnits;
  }
  return Delivery::kStepUnits; // the main view, on both axes and for both kinds
}

// The delivery step for a receiver: the form's finest fraction for an ordinary axis, a whole
// 7-bit unit for a whole-unit receiver (see kStreamStepUnits / kWholeStepUnits).
static double DeliveryStep(Delivery d)
{
  return (d == Delivery::kStepUnits) ? kWholeStepUnits : kStreamStepUnits;
}
...
static void RefreshDerived()
{
  ...
  // MODEL 3.0's two controls, through the same single source of clamping.
  g_finenessMs = Clamp(g_finenessMs, kFinenessMinMs, kFinenessMaxMs);
  g_resistance = Clamp(g_resistance, kResistMinUi, kResistMaxUi);
}
...
static void SaveSettings()
{
  ...
  // MODEL 3.0's two controls, persisted so they survive a restart. ("ease" was this control's
  // earlier name; its key is read once more below so a saved value is not lost, then cleared.)
  _snprintf(buf, sizeof(buf), "%.6f", g_finenessMs);
  SetExtState(kStateSection, "fineness", buf, true);
  SetExtState(kStateSection, "ease", "", true);
  _snprintf(buf, sizeof(buf), "%.6f", g_resistance);
  SetExtState(kStateSection, "resist", buf, true);
  SetExtState(kStateSection, "slowref", "", true); // that parameter was removed
  // The old "shape" key is cleared: that parameter was removed, and a stale value left behind
  // would be a confusing thing to find in the store later.
  SetExtState(kStateSection, "shape", "", true);
...
static void LoadSettings()
{
  ...
  if (GetExtState)
  {
    ...
    if (d && *d)
      g_dockOn = (d[0] != '0');
    // MODEL 3.0's two controls, if they were recorded. Clamped by RefreshDerived below, so a value
    // saved under the old range (the control used to be 10..300 ms) is pulled into 50..500.
    {
      double f = 0.0;
      if (LoadDouble("fineness", &f))
        g_finenessMs = f;
      else if (LoadDouble("ease", &f)) // the key this control used before it was renamed
        g_finenessMs = f;
      double fr = 0.0;
      if (LoadDouble("resist", &fr))
        g_resistance = fr;
    }
...
struct ActionSpec
{
  ...
  int command;
  Axis axis;                // which axis' glide the action belongs to
  Kind kind;                // scroll or zoom (only the MIDI editor's vertical axis cares)
  Drive drive;
  ...
  // The filter is NOT stored per entry -- it is derived from (section, axis, kind) by
  // FilterFor(), so the table cannot disagree with the rule or with the panel call sites.
  Delivery delivery() const { return FilterFor(section, axis, kind); }
};
...
static const ActionSpec kActions[] = {
    ...
    // The VERTICAL actions take whole units (FilterFor): the receiver loses anything smaller, so a
    // small notch would not move and the finest steps would jitter. This is NOT a sensitivity
    // control -- removing it was tried and made tiny moves jitter, with the feel otherwise
    // unchanged, so it is kept. The HORIZONTAL axis is fine-grained and keeps the full stream.
    {0, 988, Axis::kHorizontal, Kind::kScroll, DRIVE_REPLAY},  // View: Scroll horizontally
    {0, 977, Axis::kHorizontal, Kind::kScroll, DRIVE_REPLAY},  // View: Scroll horizontally reversed
    {0, 989, Axis::kVertical,   Kind::kScroll, DRIVE_REPLAY},  // View: Scroll vertically
    {0, 978, Axis::kVertical,   Kind::kScroll, DRIVE_REPLAY},  // Scroll vertically reversed
    // ---- main section: zooming ----
    {0, 990, Axis::kHorizontal, Kind::kZoom, DRIVE_REPLAY},    // View: Zoom horizontally
    {0, 979, Axis::kHorizontal, Kind::kZoom, DRIVE_REPLAY},    // View: Zoom horizontally reversed
    {0, 1000, Axis::kVertical,  Kind::kZoom, DRIVE_REPLAY},    // View: Zoom vertically
    {0, 1001, Axis::kVertical,  Kind::kZoom, DRIVE_REPLAY},    // Zoom vertically reversed
    // ---- MIDI editor: scroll + zoom, all through the section's own actions ----
    // The vertical axis is filtered by FilterFor: its SCROLL takes whole units, its ZOOM goes over
    // at once. The horizontal axis is continuous.
    {32060, 40430, Axis::kVertical,   Kind::kZoom,   DRIVE_REPLAY}, // Zoom vertically
    {32060, 40431, Axis::kHorizontal, Kind::kZoom,   DRIVE_REPLAY}, // View: Zoom horizontally
    {32060, 40432, Axis::kVertical,   Kind::kScroll, DRIVE_REPLAY}, // Scroll vertically
    {32060, 40433, Axis::kHorizontal, Kind::kScroll, DRIVE_REPLAY}, // View: Scroll horizontally
    {32060, 40660, Axis::kHorizontal, Kind::kScroll, DRIVE_REPLAY}, // View: Scroll horizontally rev
    {32060, 40661, Axis::kVertical,   Kind::kScroll, DRIVE_REPLAY}, // v-scroll rev
    {32060, 40662, Axis::kHorizontal, Kind::kZoom,   DRIVE_REPLAY}, // View: Zoom horizontally rev
    {32060, 40663, Axis::kVertical,   Kind::kZoom,   DRIVE_REPLAY}, // v-zoom rev
};
...
static bool ClassifyByName(KbdSectionInfo *sec, int command, ActionSpec &out)
{
  ...
  // The filter is NOT chosen here: FilterFor(section, axis, kind) is the one rule. All that is
  // needed is which axis, and whether the name says it scrolls or zooms (kUnknown when it says
  // neither, which keeps that action on the continuous stream as before).
...
// which action, which window); the motion itself lives in anim3::Glide.
// ---------------------------------------------------------------------------
struct Integrator
{
  ...
  // Whether this gesture has handed anything over yet. A whole-unit receiver (kStepUnits) acts on
  // whole units, so a gesture whose travel never reaches one would be delivered nothing at all --
  // the axis simply would not respond. When such a gesture ends, one unit is sent in the direction
  // it was going, which is the smallest move that receiver can make (AGENTS.md 21).
  ...
  anim3::Glide glide;        // MODEL 3.0: the total still owed to the action
  // DEBUG ONLY: what the model produced during this gesture and what actually reached the action,
  // so the log can say whether an axis is starved by the MODEL (resistance eating most of it) or by
  // the DELIVERY (a receiver that drops what it is fed). Magnitudes, since direction alternates.
  double dbgProduced = 0.0, dbgSent = 0.0;
  long dbgSends = 0, dbgNotches = 0;
};
...
static void ReplayAction(int section, int command, HWND hwnd, double unitsSigned)
{
  ...
  // larger into calls instead of dropping it. The amount arriving here is already a whole
  // number of deltas (see kStreamStepUnits), so it is encoded exactly, in chunks.
  const double chunk = (double)kRelIntMax;
  while (fabs(remain) >= kStreamStepUnits * 0.5)
  {
    double d = remain;
    if (d > chunk)
      d = chunk;
...
static void DeliverTravel(Integrator &g, double step); // defined below Kick
...
static void ApplyTravelNow(Integrator &g, double signedTravel)
{
  ...
  {
    // A receiver that takes a whole step at once: no glide, so the whole travel goes out now.
    // It is still measured in whole wheel deltas (converted the same way as the eased path),
    // and REAPER's own mixer handler does the moving -- just without spreading it out.
    const double deltas = signedTravel * kDeltasPerUnit;
...
// WHERE a wheel gesture is going, and how to drive it. This is the whole interface a caller needs
// to hand a gesture over: everything else about delivery is resolved inside Kick.
struct Route
{
  int section = 0;                   // KbdSectionInfo::uniqueID (0 = main)
  int command = 0;                   // the action to replay (DRIVE_REPLAY) or 0
  Axis axis = Axis::kVertical;       // which axis' state this gesture belongs to
  Kind kind = Kind::kScroll;         // only the MIDI editor's vertical axis separates these
  Drive drive = DRIVE_REPLAY;        // how the destination is reached
  HWND hwnd = nullptr;               // context window to replay the action on
};

// THE ONE ENTRY POINT for a wheel gesture. Every caller -- the action path, the arrange
// scrollbars, the track panel, the piano keys, the mixer -- comes through here with a `Route`
// that names the destination, and NOTHING ELSE about delivery. The FILTER is resolved here from
// the route, so no caller can pick a different one, and the model below only ever produces an
// amount of travel for the action to consume.
//
// `notches` is how much wheel this message reports (1.0 = one notch), always positive; the
// direction is in `sign`.
static void Kick(Integrator &g, const Route &route, double sign, double notches)
{
  if (notches <= 0.0)
    return;

  // The filter: chosen from the route, in one place (see FilterFor).
  const Delivery delivery = (route.drive == DRIVE_MCP_WHEEL)
                                ? Delivery::kStream // the mixer is driven by its own wheel message
                                : FilterFor(route.section, route.axis, route.kind);

  const bool wasActive = g.glide.Active();
  // Same operation = same drive/action/target. Deliberately NOT tied to the glide being active:
  // a roll that briefly settles still keeps building.
  const bool sameOp = g.lastDrive == route.drive && g.lastSection == route.section &&
                      g.lastCommand == route.command;

  g.drive = route.drive;
  g.section = route.section;
  g.command = route.command;
  g.replayHwnd = route.hwnd;
  g.delivery = delivery;
  g.lastDrive = route.drive;
  g.lastSection = route.section;
  g.lastCommand = route.command;

  const double unit = OneNotchUnit();
  // The whole amount this message reports, in the units the action speaks (7-bit units; one notch
  // is 15). It is added to the total the model owes the action; resistance and easing act on that
  // total, not on this message, so nothing here depends on how the device chopped the turn up.
  const double travel = sign * unit * notches;

  const double now = Now();
  if (!sameOp)
  {
    // New operation: drop any half-finished carry, so no motion from the previous operation
    // leaks into this one.
    g.accum = 0.0;
    g.glide.Reset();
  }
  ...
  if (!wasActive)
  {
    ...
    // DEBUG: a gesture is starting, so its tallies start from nothing.
    g.dbgProduced = g.dbgSent = 0.0;
    g.dbgSends = g.dbgNotches = 0;
  }

  // ---- MODEL 3.0 -------------------------------------------------------------------------
  //
  // A receiver that can only take a whole step (the MIDI editor's vertical zoom) cannot be fed a
  // spread-out trickle, so its amount goes over at once -- that is the FILTER at work, not a
  // property of the model. Every other receiver feeds the total and lets the model work it off.
  if (delivery == Delivery::kImmediate)
  {
    ApplyTravelNow(g, travel);
    g.glide.Reset();
    if (kDebugLog)
      Log("kick(whole step) drive=%d cmd=%d notches=%.2f -> travel=%.3f", (int)route.drive,
          route.command, notches, travel);
    return;
  }

  // Add it to the total owed. From here resistance and easing decide when it reaches the action;
  // this message itself is never split up or reshaped.
  g.glide.Feed(travel);
  if (kDebugLog)
  {
    g.dbgProduced += (travel < 0.0) ? -travel : travel;
    ++g.dbgNotches;
    Log("kick(fine=%.0fms resist=%.2f) drive=%d cmd=%d delivery=%d notches=%.3f -> travel=%.4f "
        "(produced %.4f in %ld) pool=%.4f",
        g_finenessMs, g_resistance, (int)route.drive, route.command, (int)delivery, notches,
        travel, g.dbgProduced, g.dbgNotches, g.glide.Owed());
  }
}
...
static void DeliverTravel(Integrator &g, double step)
{
  ...
  {
    g.accum += step;
    // MODEL 3.0 hands over a real notch's worth of travel (15 units), so the conversion is the
    // plain unit->delta ratio (8 deltas per unit): one notch in, one device notch out, exactly
    // the distance REAPER's own wheel moves the mixer (measured: one notch = one track). Whole
    // deltas are sent; the fraction is carried.
    const double deltas = g.accum * kDeltasPerUnit;
    const double whole = (deltas < 0.0) ? -floor(-deltas) : floor(deltas);
    if (whole != 0.0)
    {
  ...
  {
    g.accum += step;
    // The step is chosen by the RECEIVER (see DeliveryStep): the form's finest fraction for an
    // ordinary axis, a whole 7-bit unit for a whole-unit receiver. Below half a step the remainder
    // is CARRIED to the next frame, never rounded up, so a small amount is never inflated into a
    // jump bigger than the wheel reported.
    const double grid = DeliveryStep(g.delivery);
    ...
    if (m >= 1.0)
    {
      ...
      if (kDebugLog)
      {
        g.dbgSent += send;
        ++g.dbgSends;
        // One line per call to the action: the amount in the receiver's own unit, and the two
        // things that matter -- how many deltas it is worth, and the carry left over.
        Log("send cmd=%d delivery=%d %.5f unit (%.4f delta, step %.5f) carry=%.5f",
            g.command, (int)g.delivery, send, send * kDeltasPerUnit, grid, g.accum);
      }
    }
    return;
  }
...
// MODEL 3.0's glide is exact over the whole dt, so the motion is frame-rate independent.
// ---------------------------------------------------------------------------
static void TickIntegrator(Integrator &g, double dt)
{
  if (!g.glide.Active() || dt <= 0.0)
    return;
  const double step = g.glide.Tick(dt, AnimParams3());
...
static void Tick()
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    // A whole-unit receiver (kStepUnits) acts on whole units, so a gesture whose travel never
    // reaches one would be delivered nothing at all -- the axis simply would not respond. When such
    // a gesture ends, send one unit in the direction it was going, which is the smallest move this
    // receiver can make. A gesture that did reach a unit keeps its own remainder for the next one,
    // so ordinary use is unchanged.
    //
    // This is only for that receiver. It was briefly removed as part of a "hand nothing over that
    // the model did not produce" rule, and that is what broke the vertical axes: the model's travel
    // for a slow turn is a fraction of a unit, the whole-unit receiver dropped it, and the axis read
    // as "does not move, then suddenly moves". See Delivery.
    ...
    // DEBUG: the gesture is over, so report what it was worth. `produced` is what the MODEL gave
    // the delivery; `sent` is what actually reached the action. A large gap means the RESISTANCE
    // ate most of it; a small gap with no visible motion means the RECEIVER dropped what it got.
    if (kDebugLog && wasActive && !g.glide.Active())
      Log("GESTURE END sec=%d cmd=%d delivery=%d notches=%ld produced=%.4f sent=%.4f (%ld sends) "
          "kept=%.1f%%",
          g.section, g.command, (int)g.delivery, g.dbgNotches, g.dbgProduced, g.dbgSent,
          g.dbgSends, g.dbgProduced > 0.0 ? 100.0 * g.dbgSent / g.dbgProduced : 0.0);
  }
...
static bool OnAction(KbdSectionInfo *sec, int command, int val, int val2, int relmode, HWND hwnd)
{
  ...
  r.drive = spec.drive;
  r.hwnd = hwnd;
  Kick(spec.axis == Axis::kHorizontal ? g_horz : g_vert, r, wheelSign, notches);
  StartTimer();
  if (kDebugLog)
  ...
  if (code == HC_ACTION && wParam == PM_REMOVE)
  {
    ...
    if (m && m->message == WM_MOUSEWHEEL)
    {
      ...
      if (bar != ArrangeBar::kNone)
      {
        ...
        if (cmd != 0)
        {
          // The vertical bar's Alt gestures are the vertical ZOOM actions; FilterFor resolves the
          // right delivery from the route, exactly as it does on the action path.
          Route r;
          r.section = 0;
          r.command = cmd;
          r.axis = bar == ArrangeBar::kVertical ? Axis::kVertical : Axis::kHorizontal;
          r.kind = alt ? Kind::kZoom : Kind::kScroll;
          r.drive = DRIVE_REPLAY;
          r.hwnd = under;
          Kick(bar == ArrangeBar::kVertical ? g_vert : g_horz, r, actSign, notches);
          if (kDebugLog)
      ...
      if (g_glideOn && bar == ArrangeBar::kNone && !surface)
      {
        ...
        if (mcp)
        {
          ...
          r.section = 0;
          r.command = 0;
          r.axis = Axis::kHorizontal;
          r.kind = Kind::kScroll;
          r.drive = DRIVE_MCP_WHEEL;
          r.hwnd = mcp;
          Kick(g_horz, r, actSign, notches);
          StartTimer();
          InterlockedExchange(&g_wheelTick, 0);
      ...
      if (surface)
      {
        ...
        if (isTcp)
        {
          ...
          if (cmd != 0)
          {
            // "Scroll TCP" scrolls (continuous); "Adjust vertical zoom" is the zoom action. Both
            // are the main section's vertical axis, so FilterFor resolves them the same way the
            // action path does -- the track panel needs no rule of its own.
            Route r;
            r.section = 0;
            r.command = cmd;
            r.axis = Axis::kVertical;
            r.kind = (cmd == 1000) ? Kind::kZoom : Kind::kScroll;
            r.drive = DRIVE_REPLAY;
            r.hwnd = surface;
            Kick(g_vert, r, actSign, notches);
          }
          else
          {
            took = false;
        ...
        {
          // MIDI editor piano keys: vertical scroll via the editor's own action, with the same
          // route the note area uses -- so one rule covers the piano keys and the note area.
          HWND editor = MIDIEditor_GetActive ? MIDIEditor_GetActive() : nullptr;
          Route r;
          r.section = kMidiSection;
          r.command = kMidiScrollVert;
          r.axis = Axis::kVertical;
          r.kind = Kind::kScroll;
          r.drive = DRIVE_REPLAY;
          r.hwnd = editor;
...
//
// MODEL 3.0 has ONE control (the easing), so the panel shows one row. The table and the layout are
// unchanged from before -- only the row count dropped -- so raising this number again is how a
// second control would come back.
static const int kNumSliders = 2;
...
struct SliderSpec
{
  ...
  double step;          // smallest value change the control makes; 0 = continuous. A row that
                        // wants decimal places (Resistance: two) sets this, so its value is a
                        // round number rather than whatever the 1000-position travel lands on.
  ...
  // (MODEL 3.0's single easing has none), in which case the fields below are unused and the control
  // is not created.
  bool hasKnob;
  double *knobValue;
...
{
  double v = s.min + (s.max - s.min) * (pos / 1000.0);
  if (s.step > 0.0) // A row that asks for a step: land on whole steps from min.
    v = s.min + (double)(int)((v - s.min) / s.step + 0.5) * s.step;
  return v;
}
...
static void UpdateLabels()
{
  ...
  for (int i = 0; i < kNumSliders; ++i)
  {
    ...
    // Decimals follow the row's own step when it has one, so a 0.01 control reads as 0.01.
    const double span = (s.max - s.min) / 1000.0;
    const int dec = (s.step > 0.0) ? (s.step >= 0.1 ? 1 : 2) : (span >= 0.1 ? 1 : 2);
    ...
    // MODEL 3.0's rows have no knob, so their title is the name and value. Resistance also spells
    // out the take per step, so the number means something at a glance.
    if (s.value == &g_resistance)
      _snprintf(buf, sizeof(buf), "%s: %.*f   (%.0f ms takes %.*f delta%s)", s.name, dec,
                g_resistance, kResistStepMs, dec, g_resistance, g_resistance == 1.0 ? "" : "s");
    else if (!s.hasKnob)
...
// this size the content fits, so none is shown. No room is added for the response curve while it
// is off (see LayoutControls).
static int PanelIdealHeight(const PanelMetrics &m)
{
  return m.pad + m.headH + m.rowH * kNumSliders + m.monGap + m.monH + m.pad;
}
...
static void LayoutControls(HWND h)
{
  ...
  for (int i = 0; i < kNumSliders; ++i)
  {
    ...
    // A knob takes the far-right end of the row; the "hi" end-cap and the fader then share what
    // is left, so the knob sits behind its slider without squeezing the track to nothing. A row
    // with no knob gives the whole remaining width to the fader.
  ...
  // The response-curve block is OFF for now: MODEL 3.0's single control is a time, and the curve
  // still draws the OLD model's speed silhouette, so showing it would describe something the plugin
  // no longer does. The code is kept so it can come back with a 3.0-appropriate drawing.
  g_hasCurve = false;

  // The wheel monitor, directly below the parameter cards: it shows what the plugin is being fed,
  // for studying real mice.
  {
    const int my = y0 + m.pad + m.headH + m.rowH * kNumSliders + m.monGap;
...
static void CreatePanelChildren(HWND h)
{
  ...
  for (int i = 0; i < kNumSliders; ++i)
  {
    ...
    // The knob that sits behind this slider -- only if the row has one.
    if (s.hasKnob)
    {
...
static LRESULT CALLBACK CfgProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
  ...
  case SWSC_KNOB_RESET:
  {
    ...
    for (int i = 0; i < kNumSliders; ++i)
    {
      if (!g_knobs[i].value || g_knobs[i].id != (int)wp)
        continue;
  ...
  case SWSC_KNOB_CHANGED:
  {
    ...
    for (int i = 0; i < kNumSliders; ++i)
    {
      if (!g_knobs[i].value || g_knobs[i].id != cid)
        continue;
      g_knobPos[i] = (int)lp;
...
static void BuildSliderSpecs()
{
  static const SliderSpec kTable[kNumSliders] = {
      // MODEL 3.0's two controls. The row order is (name, value, min, max, unit, lomark, himark,
      // default, hue, step) then the knob block -- hasKnob false for both, so the knob fields are
      // unused. `step` 0 means the slider travels continuously.
      {"Fineness", &g_finenessMs, kFinenessMinMs, kFinenessMaxMs, "ms", "Direct", "Fine",
       kDefaultFinenessMs, RGB(120, 190, 255), 0.0,
       false, nullptr, 0.0, 1.0, 0.0, ""},
      // Resistance is "how many deltas are taken per 1 ms", in steps of 0.01: the fine end of the
      // range is where the useful settings are, so the decimals matter more than the top end.
      {"Resistance", &g_resistance, kResistMinUi, kResistMaxUi, "", "Light", "Heavy",
       kDefaultResist, RGB(150, 220, 150), 0.01,
       false, nullptr, 0.0, 1.0, 0.0, ""},
  };
  ...
  for (int i = 0; i < kNumSliders; ++i)
  {
    ...
    // The knob record is a VIEW of this row, so the two cannot disagree. A row without a knob
    // gets NO record at all (id 0 = "not a control"): nothing creates, lays out or handles it, so
    // there is no null value to dereference anywhere.
    if (kTable[i].hasKnob)
    {
