...
> ⚠️ **未验证**：作者手上没有无级滚轮，这一条是按两组实测数据推算的。用起来不对的话，
> 见下方「设备排查（DEV 版）」。
...
如果**无级滚轮**或**触控板**用起来不正常，可以换上 **DEV 版**跑一次，它会记录最近的滚轮参数，
把生成的记录文件发出来即可帮助定位。

* **文件名**：`reaper_smoothwheelscroll-x64-DEV.dll`（扩展列表里显示 `… 1.7.0 DEV (wheel log)`）。
* **和正式版二选一**，不要同时放进 `UserPlugins` —— 两个都装会让每个滚轮被处理两次。
* 记录文件写在**插件同目录**（`UserPlugins`）：`SmoothWheelScroll_wheel_log.txt`，
  滚动时自动覆盖、不会变大。里面**没有**项目路径、轨道名、媒体或 REAPER 偏好，可以直接打开检查后发送。

详细步骤见 [DEV_WHEEL_LOG.md](DEV_WHEEL_LOG.md)。
