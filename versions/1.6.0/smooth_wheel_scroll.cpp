...
enum
{
  ...
  IDC_L_HOLD,
  IDC_L_COAST,
  // The five knobs, one behind each slider: Start sets the rise's duration, the other four
  // bend their segment of the curve. Same order as the sliders.
  IDC_K_START = 1210,
  IDC_K_ACCEL,
  IDC_K_RELEASE,
  IDC_K_HOLD,
...
// The knob behind each slider. A knob's value is NOT the slider's: the Start knob is the
// rise's duration (ms), and the other four are their curve segment's bend. Declared here so
// the value mapping and the label code can use them before the control code below.
struct KnobSpec
{
  int id;
  double *value;
  double min, max;
  const char *name; // for the log only
};
...
static void LayoutControls(HWND h)
{
  ...
  // The master switch is set to the width its caption actually needs, not the full panel width.
  // A check box is drawn OPAQUE: it fills its whole rectangle with its own face colour, so a
  // full-width box painted a full-width band that did not match the panel behind it. Measured:
  // 380 px wide coloured out to x=391, 200 px wide stopped at x=210 -- the band follows the
  // control's rectangle. Sizing it to the caption (plus the glyph, the gap and a little slack for
  // the focus rectangle) makes the switch read as a line of text on the panel, and the background
  // handler below makes even that rectangle the panel's own colour.
  {
    const int boxW = GetSystemMetrics(SM_CXMENUCHECK);
    ...
    if (w > maxW) w = maxW;
    SetWindowPos(g_chkGlide, nullptr, m.pad, y0 + m.pad, w, m.headH, SWP_NOZORDER);
  }
...
static void CreatePanelChildren(HWND h)
{
  ...
  g_chkGlide = CreateWindowExA(0, "BUTTON",
                               ...
                               g_hInst, nullptr);
  // An owner-drawn button is painted by us (see WM_DRAWITEM) and its state is READ FROM g_glideOn,
  // NOT from the control.
  //
  // That is deliberate: BM_SETCHECK was MEASURED to do nothing on a BS_OWNERDRAW button -- setting
  // BST_CHECKED left BM_GETCHECK reporting BST_UNCHECKED, and the drawing therefore always came out
  // unchecked however the flag was flipped (the master switch worked, because g_glideOn flipped, but
  // the tick never appeared). A plain auto check box keeps that state for us; an owner-drawn one
  // does not, so the state lives in the variable the rest of the plugin already uses.
...
static void RefreshPanelTheme()
{
  ...
  // Every child repaints: the statics draw with the brush just replaced, and the owner-drawn master
  // switch (WM_DRAWITEM) is painted by us with the palette too.
...
static LRESULT CALLBACK CfgProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
  ...
  case SWSC_KNOB_RESET:
  {
    ...
    for (int i = 0; i < kNumSliders; ++i)
    {
      if (g_knobs[i].id != (int)wp)
        continue;
      static const double kDefaultPerKnob[kNumSliders] = {
          kDefaultOnsetMs, kDefaultBend, kDefaultHoldBend, kDefaultBend, kDefaultBend};
      *g_knobs[i].value = kDefaultPerKnob[i];
      g_knobPos[i] = ValueToKnob(i);
  ...
  case WM_COMMAND:
    ...
    case IDC_CHK_GLIDE:
    {
      ...
      if (g_cfgUpdating)
        return 0;
      // THE STATE IS THE VARIABLE, NOT THE CONTROL. An owner-drawn button neither toggles its own
      // check state on a click nor stores one set with BM_SETCHECK (both measured), so reading
      // BM_GETCHECK returned the same value every time and the tick could never be drawn. g_glideOn
      // is the state the rest of the plugin already uses and persists, so it is flipped directly
      // and the switch is repainted from it.
      g_glideOn = !g_glideOn;
      HWND chk = GetDlgItem(h, IDC_CHK_GLIDE);
      if (chk)
        InvalidateRect(chk, nullptr, FALSE);
      SaveSettings();
      return 0;
    }
    }
  ...
  // The master switch is OWNER-DRAWN, so it is painted here rather than by the button control.
  //
  // This is the second attempt: the first was to give the button the panel's brush through
  // WM_CTLCOLORBTN. That did not work, because a THEMED check box ignores the control colour and
  // fills its rectangle from its own theme part -- so the band that did not match the panel stayed
  // (the glyph pixels measured identical all along, which is what showed the brush was being
  // ignored rather than applied). Drawing it ourselves removes the question entirely: there is no
  // themed background left to disagree, the panel colour is used because that is what we paint,
  // and the glyph is drawn to match the themes the rest of the panel uses.
  case WM_DRAWITEM:
  {
    const DRAWITEMSTRUCT *di = (const DRAWITEMSTRUCT *)lp;
    if (!di || di->CtlID != (UINT)IDC_CHK_GLIDE)
      break;
    HDC dc = di->hDC;
    RECT rc = di->rcItem;
    // The state comes from g_glideOn, not from the control: an owner-drawn button does not store a
    // check state (BM_SETCHECK was measured to be ignored on it), so asking the control here would
    // always draw an empty box. g_glideOn is what SaveSettings/LoadSettings already use.
    const bool on = g_glideOn;
    const bool focused = (GetFocus() == di->hwndItem);

    // Background: the panel's own colour, so the switch sits on the panel with no band.
    FillRect(dc, &rc, g_theme.bgBrush);

    HGDIOBJ oldFont = SelectObject(dc, UiFont(nullptr));
    SetBkMode(dc, TRANSPARENT);

    // The glyph: a standard box, the size Windows uses for a menu check, drawn with the panel's
    // text colour. A tick is two strokes; when off the box is left empty rather than filled, which
    // is how a check box reads.
    ...
    if (on)
    {
      // The tick, inset inside the box. Drawn with a pen a little heavier than the frame so it
    ...
    // The caption, left of the glyph by the same gap the minimum width assumes.
    char text[256] = {0};
    ...
    if (focused)
    {
      RECT fr = {rc.left, rc.top, rc.right, rc.bottom};
      fr.left += 1; fr.top += 1; fr.right -= 1; fr.bottom -= 1;
      DrawFocusRect(dc, &fr);
    }
    SelectObject(dc, oldFont);
    return TRUE;
...
static void BuildSliderSpecs()
{
  ...
  // The five knobs, in the SAME order as the sliders (Start, Accel, Hold, Coast, Release).
  // Their values are separate from the sliders': the Start knob is the rise's duration, and
  // the other four are their curve segment's bend. The RANGES come from kKnobMin/kKnobMax, the same
  // table RefreshDerived clamps against, so a knob can never be laid out on a different range than
  // the one its value was clamped to.
  g_knobs[0] = {IDC_K_START, &g_onsetMs, kKnobMin[0], kKnobMax[0], "Start (rise ms)"};
  g_knobs[1] = {IDC_K_ACCEL, &g_bend[1], kKnobMin[1], kKnobMax[1], "Accel bend"};
  g_knobs[2] = {IDC_K_HOLD, &g_bend[2], kKnobMin[2], kKnobMax[2], "Hold bend"};
  g_knobs[3] = {IDC_K_COAST, &g_bend[3], kKnobMin[3], kKnobMax[3], "Coast bend"};
  g_knobs[4] = {IDC_K_RELEASE, &g_bend[4], kKnobMin[4], kKnobMax[4], "Release bend"};
  for (int i = 0; i < kNumSliders; ++i)
    g_knobPos[i] = ValueToKnob(i);
}
...
extern "C" __declspec(dllexport) int ReaperPluginEntry(HINSTANCE hInst, reaper_plugin_info_t *rec)
{
  ...
  rec->Register("ext_name", (void *)"Smooth Wheel Scroll 1.6.0");
