...
public:
  ...
  //
  // `notches` IS HOW MUCH WHEEL THIS MESSAGE REPORTS, in notches: 1.0 is one standard click.
  //
  // It used to be ignored, so every message counted as one whole notch. That is right for a notched
  // mouse, whose every message IS a whole notch, and wrong for a free-spinning wheel, which reports
  // each click as a FRACTION of one (15, 20, 25 out of WHEEL_DELTA's 120).
  //
  // Such a wheel is NOT a touchpad. Its clicks are still discrete events, and a click that produced
  // no motion would be a click the user cannot feel, so they are deliberately NOT accumulated into
  // whole notches -- each one moves by its own share. Scaling the claim AND the notch count by that
  // share keeps the total motion proportional to the wheel turned: eight one-eighth clicks add up to
  // exactly one whole-notch click. A plain notched mouse is unaffected, because its share is 1.
  void Kick(double notches, double sign, int identity, double now, const Params &P) {
    ...
    identity_ = identity;
    (void)P;
    // How much of a whole notch this message carries. A caller that describes no travel (0 or
    // negative) keeps the old meaning of one notch, so a message can never silently become a no-op.
    const double frac = (notches > 0.0) ? notches : 1.0;
    ...
    //
    // Both the count and the claim advance by the message's own share (see `notches` above), so a
    // roll of many small clicks builds up exactly as far as the same rotation in whole clicks would.
    rollN_ += frac;
