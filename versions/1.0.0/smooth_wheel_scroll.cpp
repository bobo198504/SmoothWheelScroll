...
static const double kDefaultReleaseMs = 100.0;
...
//
// true  = dense "device stream": a fast multimedia timer and 1/256-unit delivery,
//         so the receiver sees many small steps (this is what proved too dense for
//         REAPER and must not be the default).
// false = few, whole-unit steps, exactly the pre-precision output shape.
//
// Only the OUTPUT density is switched here. The integration grid stays on (see
// AnimParams) because that is correctness, not density.
// ---------------------------------------------------------------------------
static const bool kFineOutput = false;

// Animation tick for the baseline (plain thread timer). Windows runs this at about
// 15.6 ms here regardless of the requested value.
...
// arriving at once.
static const double kOnsetRatio = 0.8;
...
static UINT g_mmTimerMs = kAnimTimerMs; // period actually requested
...
static void ReplayAction(int section, int command, HWND hwnd, double unitsSigned)
{
  if (unitsSigned == 0.0)
    return;
  // Baseline: whole units only, encoded in the plain 7-bit relative form, exactly
