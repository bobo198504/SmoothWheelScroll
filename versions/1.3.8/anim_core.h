...
struct Params {
  ...
  // Feed-rate taper for speed ("escape velocity" feel). A notch's added velocity is
  // multiplied by 1/(1 + (v/softStart)^softPow), so the faster a roll already goes the
  // less each further notch adds: speed keeps climbing with no ceiling, but with ever
  // smaller steps, so a long roll eases off instead of running away.
  //
  // softStart is the KNEE: speeds well below it are essentially untouched (the factor
  // is ~1), which is what keeps ordinary rolling exactly as it was. softPow shapes how
  // sharply the taper bites once past the knee -- 1 is a gentle hyperbola, higher
  // values leave the low range flatter and cut the high range harder. 0 disables it.
  double softStart = 0.0;
  double softPow = 4.0;
...
public:
  ...
  void Kick(double unitTravel, double sign, int identity, double now, const Params &P) {
    ...
    // Escape-velocity style falloff: a notch adds less the faster the roll already
    // goes, so speed keeps rising but with a smaller and smaller gain. There is no
    // ceiling -- only the feed rate tapers -- so a roll can always get faster, just
    // ever more slowly. Speeds well below softStart are left essentially alone (that
    // is what keeps ordinary rolling unchanged), and softPow sets how hard the taper
    // bites past it. 0 disables the taper.
    double feed = 1.0;
    if (P.softStart > 0.0) {
      const double r = fabs(vel_) / P.softStart;
      const double m = (P.softPow > 0.0) ? P.softPow : 1.0;
      feed = 1.0 / (1.0 + pow(r, m));
    }
