...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// TUNING -- the feel parameters, in one place.
//
// Grouped by what a user FEELS, not by model constant, because several constants
// work together to produce one sensation: tuning "high-speed coast" means moving that
// group together, not one number. Each group's members are listed with it.
//
// These are for the author and collaborators to tune, NOT a user-facing feature:
// there is no settings window and no config file (the released plugin deliberately
// has no settings entry -- see AGENTS.md section 16). To change the feel, edit the
// values here and rebuild.
//
// The model's SHAPE is not here and must not be tuned: kFrictionPow, kOnsetRatio and
// kBurstGapMs stay below with the internals, because they define the physics (the
// release curve, the onset window, and what counts as one roll). See AGENTS.md
// sections 7 and 12. Only the five groups below are meant to move.
//
//   1. START      length of the FIRST notch's travel, in % of one wheel notch
//   2. ACCEL      added to each further notch's travel, same unit
//   3. RELEASE    time one notch takes to settle (the brake)
//   4. HIGH-HOLD  how strongly a fast roll is held back from running away
//   5. HIGH-COAST how much longer a fast roll coasts after the last notch
//
...
// --- 4. HIGH-SPEED HOLD -------------------------------------------------------
// Three mechanisms keep a fast roll from accelerating without bound. They overlap on
// purpose (see below), so this is one group: softening the hold means moving all of
// them, not one.
//   kTempoRefMs + kBrakeRelaxMax : the brake-by-rhythm (a faster roll loosens the
//       brake); this is the original build-up mechanism.
//   kSoftStart + kSoftPow       : the feed-rate taper (a notch adds less the faster
//       the roll already goes). kSoftStart is the knee -- below it nothing changes,
//       which is what keeps ordinary rolling untouched -- and kSoftPow how sharply it
//       bites past the knee.
static const double kTempoRefMs = 110.0;
static const double kBrakeRelaxMax = 4.0;
static const double kSoftStart = 1100.0;
static const double kSoftPow = 2.5;

// --- 5. HIGH-SPEED COAST ------------------------------------------------------
// The brake is divided by a factor that grows with speed and saturates, so a fast
// roll coasts longer. kRelFloor is a dead zone -- below it the boost is exactly zero,
// which is what leaves the single notch and slow rolls bit-for-bit unchanged --
// kRelGain is how far the brake can be divided at the top, and kRelRef the speed above
// the floor at which half of that is reached.
